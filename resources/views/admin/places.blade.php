            ...
            <form action="{{route('admin/add_places')}}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-4 px-4 my-2">
                    <select id="countries" name="arrival" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        ...
                        <option value="{{ $item->id_places }}">{{ $item->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-4 px-4 my-2">
...
<div id="editModal-{{$editdata->id_places}}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-2xl max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <!-- Modal body -->
            <form action="{{url('/edit_places'. $editdata->id_places)}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="mb-4 px-4 my-2">
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="title" value="{{$editdata->title}}" name="title" type="text" placeholder="Place">
                </div>
                <div class="mb-4 px-4 my-2">
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="map" value="{{$editdata->maps}}" name="map" type="text" placeholder="Map Location">
                </div>
                <div class="mb-4 px-4 my-2">
                    <textarea id="desc" name="desc" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="Description Place" cols="30" rows="10"></textarea>
                </div>
                ...
                <!-- Modal footer -->
                <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b">
                    <button data-modal-target="editModal" data-modal-toggle="editModal-{{$editdata->id_places}}" class="w-20 justify-center inline-flex text-white bg-[#050708] hover:bg-[#050708]/90 focus:ring-4 focus:outline-none focus:ring-[#050708]/50 font-medium rounded-lg text-sm px-3 py-2.5 text-center items-center mr-2 mb-2" type="submit">
                        Update
                    </button>
                </div>
            </form>
...
{{-- end modal edit --}}
{{-- modal delete --}}
<div id="deleteModal-{{$editdata->id_places}}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-2xl max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow">
            <!-- Modal header -->
            <div class="flex items-start justify-between p-4 border-b rounded-t">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center" data-modal-hide="deleteModal-{{$editdata->id_places}}">
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                    </svg>
                    <span class="sr-only">Close modal</span>
            ...
            <form action="{{url('/delete_places'. $editdata->id_places)}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="mb-4 px-4 my-2">
                ...
                <!-- Modal footer -->
                <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b">
                    <button data-modal-target="deleteModal" data-modal-toggle="deleteModal-{{$editdata->id_places}}" class="w-20 justify-center inline-flex text-white bg-[#050708] hover:bg-[#050708]/90 focus:ring-4 focus:outline-none focus:ring-[#050708]/50 font-medium rounded-lg text-sm px-3 py-2.5 text-center items-center mr-2 mb-2" type="submit">
                        Delete
                    </button>
                </div>
            </form>
